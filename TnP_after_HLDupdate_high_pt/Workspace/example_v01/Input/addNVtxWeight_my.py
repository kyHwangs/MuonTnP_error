...
parser.add_argument("-hr", "--histogramRange", default="150,-0.5,149.5", help="Range of control histogram given as \"bins,min,max\"")
...
# weights = [1.0]*(hMC.GetNbinsX())
# for i in range(0, len(weights)):

#     i_bin = i+1
#     nVtx = i # -- #vtx of 1st bin = 0 -- #
    
#     nMC = hMC.GetBinContent(i_bin)
#     nData = hData.GetBinContent(i_bin)

#     if nData == 0:
#         weights[nVtx] = 0
#     elif nMC > 0:
#         weights[nVtx] = nData/nMC
#     else:
#         weights[nVtx] = 1.0

#     print "#vtx=%d, weight=%d" % (nVtx, weights[nVtx])
...
for i in range(numEvents):
    chainMC.GetEntry(i)
    # nVtx = int(getattr(chainMC, args.branch))
    # print "%d event, nVtx = %d" % (i, nVtx)
